...
openFDA Case Study for AstraZeneca Oncology Data Scientist Interview, Question 1
Are different adverse events reported in different countries?

To find out what adverse events are associated with the specific country a user inputs
...
outcsv = 'country.csv' # output file name
outfig = 'country.png' #output figure name
country= 'US'# user specificed disease
iterations= 6 # the number of adverse events 

 
query = 'https://api.fda.gov/drug/event.json?search=occurcountry.exact:'+country+'&count=patient.reaction.reactionmeddrapt.exact' # query openFDA API
...
# plot the bar plot
objects=adv_event
objects = [ '\n'.join(wrap(l, 10)) for l in objects ]
...
plt.title('The Top {} Adverse Events for {}' .format(iterations, country), fontsize=20)
plt.savefig(outfig)
